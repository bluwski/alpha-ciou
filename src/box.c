...
float box_giou(box a, box b)
{
    ...
    float u = box_union(a, b);
    float giou_term = pow((c - u) / c, alpha_giou);
#ifdef DEBUG_PRINTS
...
float box_ciou(box a, box b)
{
    ...
    float d = pow(u,alpha_ciou) / pow(c,alpha_ciou);
    float ar_gt = b.w / b.h;
    ...
    float ciou_term = pow(d, alpha_ciou) + pow(alpha * ar_loss, alpha_ciou);                   //ciou
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, riou_term: %f\n", c, u, ciou_term);
#endif
...
//#endif
    ...
    int alpha_giou = 3;
      if (giou_C > 0) {
        ...
        p_dt += (pow((giou_C * dU_wrt_t) - (U * dC_wrt_t)) / (giou_C * giou_C),alpha_giou);
        p_db += (pow((giou_C * dU_wrt_b) - (U * dC_wrt_b)) / (giou_C * giou_C),alpha_giou);
        p_dl += (pow((giou_C * dU_wrt_l) - (U * dC_wrt_l)) / (giou_C * giou_C),alpha_giou);
        p_dr += (pow((giou_C * dU_wrt_r) - (U * dC_wrt_r)) / (giou_C * giou_C),alpha_giou);
      }
	  if (Iw<=0||Ih<=0) {
		p_dt = (pow((giou_C * dU_wrt_t) - (U * dC_wrt_t)) / (giou_C * giou_C),alpha_giou);
        p_db = (pow((giou_C * dU_wrt_b) - (U * dC_wrt_b)) / (giou_C * giou_C),alpha_giou);
	...
	float ar_dw=8/(M_PI*M_PI)*(atan(ar_gt)-atan(ar_pred))*pred.h;
        ...
        if (C > 0) {
        // dar*
            p_dx += (pow(2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S),alpha_ciou) / (pow(C * C, alpha_ciou));
            p_dy += (pow(2*(truth.y-pred.y)*C-(2*Cw*dCw_dy+2*Ch*dCh_dy)*S),alpha_ciou) / (pow(C * C, alpha_ciou));
            p_dw += (pow(2*Cw*dCw_dw+2*Ch*dCh_dw)*S,alpha_ciou) / (pow(C * C,alpha_ciou)) + (pow(alpha * ar_dw,alpha_ciou));
            p_dh += (pow(2*Cw*dCw_dh+2*Ch*dCh_dh)*S,alpha_ciou) / (pow(C * C,alpha_ciou)) + (pow(alpha * ar_dh,alpha_ciou));
        }
	if (Iw<=0||Ih<=0){
            p_dx = (pow(2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S),alpha_ciou) / (pow(C * C,alpha_ciou));
            p_dy = (pow(2*(truth.y-pred.y)*C-(2*Cw*dCw_dy+2*Ch*dCh_dy)*S),alpha_ciou) / (pow(C * C,alpha_ciou));
            p_dw = (pow(2*Cw*dCw_dw+2*Ch*dCh_dw)*S,alpha_ciou) / (pow(C * C) + alpha * ar_dw, alpha_ciou);
            p_dh = (pow(2*Cw*dCw_dh+2*Ch*dCh_dh)*S,alpha_ciou) / (pow(C * C) + alpha * ar_dh,alpha_ciou);
        }
    }
